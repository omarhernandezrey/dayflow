...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, UserEntity>> register({
    ...
    try {
      ...
      final existing = await _db.queryUsers(where: 'email = ?', whereArgs: [email.trim().toLowerCase()]);
      if (existing.isNotEmpty) {
        return const Left(ValidationFailure('Ya existe una cuenta con este correo'));
      }
  ...
  Future<Either<Failure, UserEntity>> login({
    ...
    try {
      ...
      if (rows.isEmpty) {
        return const Left(ValidationFailure('Correo o contraseña incorrectos'));
      }
  ...
  Future<Either<Failure, void>> logout() async {
    try {
      await _db.rawQuery('DELETE FROM settings WHERE key = ?', [_sessionKey]);
  ...
  Future<Either<Failure, UserEntity?>> getCurrentUser() async {
    try {
      ...
      final rows = await _db.queryUsers(where: 'id = ?', whereArgs: [userId]);
